...
fn sync_opencode(state: State<'_, AppState>) -> Result<String, String> {
    ...
    info!("Sync complete: {} records upserted, {} total in database", synced, total_records);
    Ok(format!("Synced {} records ({} total)", synced, total_records))
}

#[tauri::command]
fn get_dashboard(state: State<'_, AppState>, days: Option<i64>) -> Result<DashboardData, String> {
    let db = state.db.lock().map_err(|e| e.to_string())?;

    let summary = db.get_usage_summary(days).map_err(|e| e.to_string())?;
    let provider_usage = db.get_provider_usage(days).map_err(|e| e.to_string())?;
    let model_usage = db.get_model_usage(days).map_err(|e| e.to_string())?;
    let activity = db.get_activity(20).map_err(|e| e.to_string())?;
    drop(db);
