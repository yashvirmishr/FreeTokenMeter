...
impl Database {
    pub fn new(db_path: &PathBuf) -> Result<Self, rusqlite::Error> {
        ...
        // Schema v2: added reasoning_tokens, removed dead time_updated column.
        ...
        // Migration: if old v1 table exists without reasoning_tokens, add the column
        let has_reasoning = {
            conn.prepare("SELECT reasoning_tokens FROM usage_records LIMIT 1").is_ok()
        };
        if !has_reasoning {
            info!("Migrating usage_records schema: adding reasoning_tokens column");
            conn.execute_batch(
        ...
        self.conn.execute(
            "INSERT OR REPLACE INTO usage_records
             (id, provider, model, timestamp, input_tokens, output_tokens, reasoning_tokens,
              cached_tokens, total_tokens, estimated_cost, session_id, time_created)
    ...
    pub fn get_usage_summary(&self, days: Option<i64>) -> Result<UsageSummary, rusqlite::Error> {
        ...
        if let Some(row) = rows.next() {
            let (total, input, output, reasoning, cached, count, est_cost) = row?;
            // actual_cost: sum of sessions where OpenCode reported a real cost
            // For free tools, this is always $0
            let actual_cost = self.get_actual_cost(days)?;
            Ok(UsageSummary {
                total_tokens: total,
                input_tokens: input,
    ...
    /// Get the sum of costs that OpenCode actually charged (> $0).
    fn get_actual_cost(&self, days: Option<i64>) -> Result<f64, rusqlite::Error> {
        let (where_clause, query_params): (String, Vec<Box<dyn rusqlite::types::ToSql>>) = match days {
            Some(d) => {
                let cutoff = Utc::now().timestamp_millis() - (d * 24 * 60 * 60 * 1000);
                ("WHERE time_created >= ?1 AND estimated_cost > 0".to_string(), vec![Box::new(cutoff)])
            }
            None => ("WHERE estimated_cost > 0".to_string(), vec![]),
        };

        let query = format!(
            "SELECT COALESCE(SUM(estimated_cost), 0.0) FROM usage_records {}",
            where_clause
        );
    ...
    pub fn get_provider_usage(&self, days: Option<i64>) -> Result<Vec<ProviderUsage>, rusqlite::Error> {
        ...
        let query = format!(
            "SELECT provider, COALESCE(SUM(total_tokens), 0) as tokens, COALESCE(SUM(estimated_cost), 0.0) as cost
        ...
        let raw: Vec<(String, i64, f64)> = rows.filter_map(|r| r.ok()).collect();
        let total_tokens: i64 = raw.iter().map(|(_, t, _)| *t).sum();
        
        Ok(raw.into_iter().map(|(provider, tokens, cost)| {
    ...
    pub fn get_model_usage(&self, days: Option<i64>) -> Result<Vec<ModelUsage>, rusqlite::Error> {
        ...
        let query = format!(
            "SELECT model, COALESCE(SUM(total_tokens), 0) as tokens, COALESCE(SUM(estimated_cost), 0.0) as cost
        ...
        Ok(raw.into_iter().map(|(model, tokens, cost)| {
            let percentage = if total_tokens > 0 {
                (tokens as f64 / total_tokens as f64) * 100.0
            } else {
                0.0
